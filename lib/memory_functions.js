...
exports.vram_map = function(addr) {
  ...
  if(addr === 0x3F10|| addr === 0x3F14 || addr === 0x3F18|| addr === 0x3F1C){
    //console.log(Number(this.PC).toString(16)+" vmap "+Number(addr).toString(16)+" -> "+Number(0x3F00).toString(16));
    return addr-0x10;
  }
...
exports.write_memory=function(addr, byte) {
  ...
  if(addr === constants.PPUCTRL) {
    ...
    this.cycles+=513;
    for(var i = 0; i < 256; i++) this.oam[(this.oamaddr++)&0xff] = this.memory_cpu[0x100*byte+i];
    return true;
