...
exports.getColor = function(pixel,palette,sprite) {
  return this.memory_ppu[0x3F00+pixel+(palette<<2)+(sprite<<4)];
}
...
exports.ppu_draw_internal = function(pixels) {
  ...
  if (!this.render_ppu_nametable) {
    ...
    if (background) {
      ...
      const selected_tile_table = (this.memory_cpu[0x2000] & (1 << 4)) ? 0x1000:0;

      var scroll_y = this.ppu_scroll_y;
      for(var tilerow = Math.floor(scroll_y/8); tilerow < 30 + Math.floor(scroll_y/8); tilerow++)
      {
        var scroll_x = this.ppu_scroll_x_per_line[tilerow*8];
        for(var tilecol = Math.floor(scroll_x/8); tilecol < 32+Math.floor(scroll_x/8); tilecol++)
        {
          var tiletable_temp = tileTable;
          var tilecol_temp = tilecol;
          if (tilecol_temp >= 32) {
            tilecol_temp -= 32;
            if(tileTable===0x2000||tileTable===0x2800) tiletable_temp += 0x400;
            else tiletable_temp -= 0x400;
          }

          var tilerow_temp = tilerow;
          if (tilerow_temp >= 30) {
            tilerow_temp -= 30;
            if(tileTable===0x2000||tileTable===0x2400) tiletable_temp += 0x800;
            else tiletable_temp -= 0x800;
          }

          var sprite = this.memory_ppu[tiletable_temp + tilerow_temp * 32 + tilecol_temp];
          var tile = this.getTile(sprite, selected_tile_table);
          const palette_addr = tiletable_temp + 0x3C0;

          var colorTable = this.memory_ppu[palette_addr + (Math.floor(tilerow_temp / 4) * 8) + Math.floor(tilecol_temp / 4)];
          var colors = (colorTable >> ((Math.floor((tilerow_temp / 2)) & 1) * 4 + (Math.floor((tilecol_temp / 2)) & 1) * 2)) & 0x3;

          tilerow_temp-=Math.floor(scroll_y/8);
          tilecol_temp-=Math.floor(scroll_x/8);

          for (var y = 0; y < 8; y++) {
            for (var x = 0; x < 8; x++) {
              var index = ((tilerow * 8 + y - (scroll_y & 7)) * 256 + (tilecol_temp * 8) + x - (scroll_x & 7)) * 4;
              var color = this.getColor(tile[y * 8 + x], colors, 0);
              var pixel;
              if(tile[y * 8 + x] === 0) pixel=background_color;
              else pixel = this.color_map[color & 0x3f];
