  ...
  if (!this.render_ppu_nametable) {
    ...
    if (background) {
      ...
      for (let i = 0; i < tiles; ++i) {
        var sprite = this.memory_ppu[tileTable + i];
      ...
      for (var i = 0; i < 256; i += 4) {
        ...
        for (var y = 0; y < 8; y++) {
          for (var x = 0; x < 8; x++) {
            ...
            if(tileColor) {
              var pixel = this.color_map[color];
              pixels[index] = pixel[0];
              pixels[index + 1] = pixel[1];
              pixels[index + 2] = pixel[2];
              pixels[index + 3] = 255;
    ...
    for (let i = 0; i < tiles; ++i) {
      ...
      for (var y = 0; y < 8; y++) {
        for (var x = 0; x < 8; x++) {
          var index = ((tilerow * 8 + y) * 256 + (tilecol * 8) + x) * 4;
          var pixel = this.color_map[this.getColor(tile[y * 8 + x], colors, 0)];
          pixels[index] = pixel[0];
          pixels[index + 1] = pixel[1];
          pixels[index + 2] = pixel[2];
          pixels[index + 3] = 255;
    ...
    for (var palette_idx = 0; palette_idx < 4; palette_idx++) {
      for (var y = 0; y < 10; y++) {
        for (var x = 0; x < 10; x++) {
          ...
          var pixel = this.color_map[this.getColor(palette_idx, palette_table, 0)];
          pixels[index] = pixel[0];
          pixels[index + 1] = pixel[1];
          pixels[index + 2] = pixel[2];
          pixels[index + 3] = 255;
